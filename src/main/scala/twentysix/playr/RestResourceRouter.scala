...
abstract class AbstractRestResourceRouter[C <: BaseResource: ResourceWrapper] {
  ...
  class SubResourceRouting(val router: SubRestResourceRouter[C, _]) extends Routing[C] {
    def routing(
      ...
      def next(id: C#IdentifierType) =
        Include {
          val subRouter =
            router.withParent(controller, id, RouteFilterContext(name, Some(sid), Some(id), parentContext), filter)
  ...
  class ActionRouting(
      val actions: PartialFunction[HttpMethod, ResourceAction[C]],
      route: String,
      attributes: Map[HttpMethod, Map[Any, Any]]
  ) extends Routing[C] {
    ...
    def routing(
      ...
      val next: (C#IdentifierType) => Option[EssentialAction] =
        HttpMethod.All
          .get(requestHeader.method)
          .map {
            actions.andThen { action => id: C#IdentifierType =>
              ...
              case _ => id: C#IdentifierType => Some(controller.Action { Results.MethodNotAllowed })
            }
          }
          .getOrElse { id =>
            Some(controller.Action { Results.MethodNotAllowed })
          }
      ApplyRouterFilter(
        filter,
        Custom,
        RouteFilterContext.pathWithParent(parentContext, s"$name/$route"),
        requestHeader
      ) { () =>
  ...
  def add(route: String, attributes: Map[HttpMethod, Map[Any, Any]])(
      actions: PartialFunction[HttpMethod, ResourceAction[C]]
  ): this.type =
  ...
  def addSubRouter[S <: BaseResource: ResourceWrapper](route: String, factory: ControllerFactory[C, S])(
      block: SubRestResourceRouter[C, S] => SubRestResourceRouter[C, S]
  ): this.type =
...
class RestResourceRouter[C <: BaseResource: ResourceWrapper](
...
) extends AbstractRestResourceRouter[C]
    with RestRouter
    with SimpleRouter {
  ...
  private val methodNotAllowed = controller.Action { Results.MethodNotAllowed }
  ...
  def optionsRoutingHandler(map: Map[ResourceCaps.Value, String]) = controller.Action {
    ...
    path match {
      ...
      case "" | "/" =>
        method match {
          ...
          case "POST" =>
            filterRoute(Create, () => wrapper.createWrapper(controller, requestHeader, name, parentContext))
      ...
      case IdExpression(sid) =>
        method match {
          case "GET" =>
            filterRoute(Read, () => wrapper.readWrapper(controller, sid, requestHeader, name, parentContext))
          case "PUT" =>
            filterRoute(Write, () => wrapper.writeWrapper(controller, sid, requestHeader, name, parentContext))
          case "DELETE" =>
            filterRoute(Delete, () => wrapper.deleteWrapper(controller, sid, requestHeader, name, parentContext))
          case "PATCH" =>
            filterRoute(Update, () => wrapper.updateWrapper(controller, sid, requestHeader, name, parentContext))
...
class SubRestResourceRouter[P <: BaseResource, C <: BaseResource: ResourceWrapper](
    val name: String,
    val factory: ControllerFactory[P, C]
) extends AbstractRestResourceRouter[C] {
  var routeMap = Map[String, Routing[C]]()
  ...
  def withParent(
      parent: P,
      id: P#IdentifierType,
      context: RouteFilterContext[P#IdentifierType],
      parentFilter: Option[RestRouterFilter]
  ) = new RestResourceRouter[C](
