...
trait DefaultFilteredWriteResourceWrapper extends DefaultWriteResourceWrapper{
  implicit def defaultFilteredImpl[T<:BaseResource with ResourceRouteFilter] = new WriteResourceWrapper[T] with DefaultCaps {
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) =
      obj.routeFilter.filterWrite( requestHeader,
...
object WriteResourceWrapper extends DefaultFilteredWriteResourceWrapper{
  ...
  implicit def writeFilteredResourceImpl[T<:BaseResource with ResourceWrite with ResourceRouteFilter] = new WriteResourceWrapper[T]{
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) = {
