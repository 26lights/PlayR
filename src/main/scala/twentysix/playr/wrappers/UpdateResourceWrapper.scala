...
trait DefaultFilteredUpdateResourceWrapper extends DefaultUpdateResourceWrapper{
  implicit def defaultFilteredImpl[T<:BaseResource with ResourceRouteFilter] = new UpdateResourceWrapper[T] with DefaultCaps {
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) =
      obj.routeFilter.filterUpdate( requestHeader,
...
object UpdateResourceWrapper extends DefaultFilteredUpdateResourceWrapper{
  ...
  implicit def updateFilteredResourceImpl[T<:BaseResource with ResourceUpdate with ResourceRouteFilter] = new UpdateResourceWrapper[T]{
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) = {
