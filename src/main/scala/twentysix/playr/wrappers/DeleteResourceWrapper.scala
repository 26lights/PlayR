...
trait DefaultFilteredDeleteResourceWrapper extends DefaultDeleteResourceWrapper{
  implicit def defaultFilteredImpl[T<:BaseResource with ResourceRouteFilter] = new DeleteResourceWrapper[T] with DefaultCaps {
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) =
      obj.routeFilter.filterDelete( requestHeader,
...
object DeleteResourceWrapper extends DefaultFilteredDeleteResourceWrapper{
  ...
  implicit def deleteFilteredResourceImpl[T<:BaseResource with ResourceDelete with ResourceRouteFilter] = new DeleteResourceWrapper[T]{
    def apply(obj: T, sid: String, requestHeader: RequestHeader, path: String, parentContext: Option[RouteFilterContext[_]]) = {
