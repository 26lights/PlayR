...
class GlobalFilterRouterTest extends FunSpec with Matchers with PlayRApp{
  ...
  implicit val filter = new RestRouterFilter() {
    def filter = {
      case RouterFilterContext(actionType, path) => (requestHeader: RequestHeader, next: () => Option[Handler]) =>
        next().map {
  ...
  describe("A RestResourceRouter with a global filter") {
    it("should return NotFound for an unexpected resource id get"){ withFilteredApp(new TestControllerAll()) { app =>
      val Some(result) = route(app, FakeRequest("GET", "/test"))
      status(result) should be(NOT_FOUND)
    }}
    it("should return Ok(read) for an expected resource id get"){ withFilteredApp(new TestControllerAll()) { app =>
    ...
    it("should return Ok('hello world') an expected resource id hello extension"){ withFilteredApp(extResourceRouter) { app =>
